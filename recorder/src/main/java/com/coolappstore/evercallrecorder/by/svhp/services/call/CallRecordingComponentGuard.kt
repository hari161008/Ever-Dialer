 ...
 * receiver, the InCallService detection path, and the notification-listener service used for
 * WhatsApp/Telegram call detection — fully enabled/disabled in lockstep with what the user has
 * actually turned on, so nothing from this module sits resident in memory / drains battery for a
 ...
 *   AND that the user hasn't switched to the [AppPreferences.CallDetectionMode.IN_CALL_SERVICE] method.
 * - [AppInCallService] is only bound by the OS while enabled, so it's kept disabled unless the
 *   master switch is on AND the user has opted into [AppPreferences.CallDetectionMode.IN_CALL_SERVICE].
